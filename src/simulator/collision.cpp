...
//  std::mutex  sim_mutex_;
  ...
  void fclCollision(float dx,float dy, float dz, size_t& num_contacts) {

    cout<<"Configure robot_ee geometry."<<endl;
    Loader scene;
...
//    <<" geom.get.aabb_radius: "<<geom.get()->aabb_radius<<endl;
    ...
    X_WBV.translation() << 393+dx, 0+dy, 642+dz;
...
//  X_WS.translation() << -365, 0, -642;
    ...
    X_WS.translation() << 560, 0, 642;
//    X_WS.linear() << 0,0,1, 0,1,0, -1,0,0;//y axis 90
    auto Sphere = new CollisionObjectf(geom, X_WS);
    ...
    cout<<"test"<<endl;
    num_contacts = fcl::collide(Sphere, Robot, Request, Result);//collision
...
//    float x{0} ,y{0}, z{0};
   ...
   /* //thread
    while(!num_contacts){
      X_WBV = fcl::Transform3<float>::Identity();
      //  X_WBV.translation() << -365, 0, -642;

      X_WBV.translation() << x,y,z;

      //  X_WBV.linear() << 0,0,1, 0,1,0, -1,0,0;//y axis 90
      Robot = new CollisionObjectf(geom, X_WBV);

      X_WS = fcl::Transform3<float>::Identity();
//  X_WS.translation() << -365, 0, -642;
      X_WS.translation() << 0, 0, 0;
//    X_WS.linear() << 0,0,1, 0,1,0, -1,0,0;//y axis 90
      Sphere = new CollisionObjectf(geom, X_WS);
...
//  auto InitCollision(float& _dx,float& _dy, float& _dz, size_t& num_contacts)->void {
//    if (sim_thread_.joinable()) {
//      return;
//    } else {
//      sim_thread_ = std::thread(fclCollision,_dx,_dy,_dz,std::ref(num_contacts));
//    }
//  }
