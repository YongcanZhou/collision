	...
	ui(new Ui::MainWindow)
{
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr cloud = std::make_shared<pcl::PointCloud<pcl::PointXYZ>>(pcl::PointCloud<pcl::PointXYZ>()),
	...
	std::shared_ptr<pcl::visualization::PCLVisualizer> viewer = std::make_shared<pcl::visualization::PCLVisualizer>(pcl::visualization::PCLVisualizer("viewer", false));
	...
	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> sources_cloud_color(cloud, 250, 0, 0); //设置源点云的颜色为红色
	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> target_cloud_color(cloud_icp, 0, 250, 0);  //目标点
	...
	viewer->addPointCloud(cloud, sources_cloud_color, "sample cloud1", v1);//添加点云pointCloudPtr，其处理结果rgb，添加到视图中，sample cloud1为点云的名字，v1代表是添加到哪个视图窗口（pcl中可设置多窗口模式）
	...
	pcl::PointCloud<pcl::PointXYZ>::Ptr Final;   // 存储配准变换源点云后的点云
	Final.reset(new pcl::PointCloud<pcl::PointXYZ>);
	pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> final_cloud_color(Final, 0, 250, 0);  //目标点
